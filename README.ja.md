...
**注意**: `.rubocop.cookpad-styleguide.yml`ファイルはMITライセンスの対象外です。このファイルは[Style guides in COOKPAD](https://github.com/cookpad/styleguide)の一部であり、[CC BY 3.0](https://creativecommons.org/licenses/by/3.0/)ライセンスの下で提供されています。

## 免責事項

**重要: このツールを使用する前に必ずお読みください。**

- このツールは、設定とコマンドに基づいてファイルを自動的に移動・削除します。
- 作成者は、このツールの使用によって生じたデータ損失、ファイル破損、その他の損害について、**ソフトウェアのバグや欠陥に起因する損害を含め、一切の責任を負いません**。
- このソフトウェアのバグや欠陥により予期しない動作やデータ損失が発生した場合でも、作成者は一切の責任を負いません。
- このツールを実行する前に、重要なファイルやディレクトリの**バックアップを必ず作成**してください。
- 実際の実行前に `--dry-run` モードを使用して変更内容をプレビューすることを強く推奨します。
- 重複検出・削除機能は、ファイルを完全に削除します。十分注意してご使用ください。
- このツールは「現状のまま」提供され、商品性、特定目的への適合性、権利非侵害性を含むがこれに限定されない、明示的または黙示的ないかなる保証もありません。
- **自己責任でご使用ください。** このツールを使用した結果について、すべての責任は使用者が負うものとします。
